...
class Accounts_receivable extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $query = $this->db->get();
        $customer_data = $query->result();
        $options = "<option value=''>-- Select --</option>";
        foreach ($customer_data as $value) {
            $name = $this->custom->getSingleValue('master_customer', 'name', ['code' => $value->customer_code]);
            $options .= "<option value='".$value->customer_code."'>";
            $options .= '('.$value->customer_code.') '.$name;
            $options .= '</option>';
        }
        $this->body_vars['customers'] = $options;

        $this->body_file = 'accounts_receivable/options.php';
    }
    ...
    public function reports()
    {
        ...
        $order_by = null;
        $order_by_many = null;
        $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
        $list = $query->result();
        $options = '<option value="">Select</option>';
    ...
    public function batch_ob_create()
    {
        ...
        $this->body_vars['save_url'] = 'accounts_receivable/save_batch_ob';
    }
        ...
        if ($post) {
            ...
            $html .= '<tr>
                            ...
                            <td style="border: none; text-align: right; vertical-align: bottom">
                                <br /><h4>DEBTOR STATEMENT</h4><br /><br /><br />
            ...
            $html .= '<table class="table" width="100%">
                        <tr>
                            <td style="text-align: left; border: none">
                                ...
                                '.$this->custom->populateCustomerAddress($customer).'
                            </td>
                        </tr>
                    </table><br />';

            // table of statements
            $html .= '<table class="table table-hover" style="width: 100%; min-width: 1100px">
                <thead>
                    <tr>
            ...
            $entries = 0;
            $table = 'accounts_receivable';
            $columns = '*, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount';
            $where = ['customer_code' => $customer->code, 'offset' => 'n'];
            ...
            $file = 'debtor_'.date('YmdHis').'.pdf';
            
            $style = $this->custom->populateMPDFStyle();
            $document = $style.$header.$html;
            
            $this->custom->printMPDF($file, $document);

        } else {
            redirect('/accounts_receivable/reports');
        }
    }
    ...
    public function print_historical_statement()
    {
        ...
        if ($post) {
            ...
            $html .= '<tr>
                            ...
                            <td style="border: none; text-align: right; vertical-align: bottom">
                                <br /><h4>HISTORICAL DEBTOR STATEMENT</h4> <br /><br /><br />
            ...
            $html .= '<br />'.$customer->name.' ('.$customer->code.')<br />';
            $html .= $this->custom->populateCustomerAddress($customer);
            ...
            $table = 'accounts_receivable';
            $columns = '*, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount';
            $where = ['customer_code' => $customer->code, 'doc_date <' => $from_date];
            $group_by = 'REPLACE(doc_ref_no, "_sp_1", "")';
            $order_by = 'doc_date';
            $order_by_many = null;
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
            ...
            $table = 'accounts_receivable';
            $columns = '*, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount';
            $where = ['customer_code' => $customer->code, 'doc_date >= ' => $from_date, 'doc_date <= ' => $to_date];
            $group_by = 'REPLACE(doc_ref_no, "_sp_1", "")';
            $order_by = 'doc_date';
            $order_by_many = null;
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
            ...
            $file = 'hist_debtor_'.date('YmdHis').'.pdf';
    ...
    {    
        ...
        // customer code list by currency selected
        $i = 0;
        $table = 'accounts_receivable';
        ...
        foreach ($customers as $customer_record) {
            // customer details
            $customer_data = $this->custom->getSingleRow('master_customer', ['code' => $customer_record->customer_code]);

            // currency details
            $currency_data = $this->custom->getSingleRow('ct_currency', ['currency_id' => $customer_data->currency_id]);
            $customer_currency_code = $currency_data->code;
            $customer_currency_rate = $currency_data->rate;

            $html .= '<tr><td colspan="6" height="20" style="border: none"></td></tr>';

            // display customer details - d-none row added to fix the font issues
            $html .= '<tr>';
            $html .= '<td colspan="6" style="border: none">';
            $html .= '<strong>Customer Name & Code : </strong>';
            $html .= $customer_data->name.' ('.$customer_data->code.')';
            if ($customer_data->contact_person !== '') {
                $html .= '<br /><strong>Contact Person : </strong>';
                $html .= $customer_data->contact_person;
                $html .= '<br />';
            }
            if ($customer_data->phone !== '') {
                $html .= '<strong>Phone : </strong>';
                $html .= $customer_data->phone;
                $html .= '<br />';
            }
            $html .= '</td>';
            $html .= '</tr>';
            ...
            $table = 'accounts_receivable';
            $columns = '*, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount';
            $where = ['customer_code' => $customer_record->customer_code, 'offset' => 'n'];
            $group_by = 'REPLACE(doc_ref_no, "_sp_1", "")';
            $order_by = null;
            $order_by_many = 'doc_date ASC, doc_ref_no ASC';
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
            ...
            foreach ($stmt_list as $stmt_record) {
                ...
                $html .= '</tr>';
                ++$i;
            } // statement per customer loops ends
    ...
    public function print_listing()
    {
        ...
        if ($post) {
            ...
            $html .= '<tr>
                            ...
                            <td style="border: none; text-align: right; vertical-align: bottom;">
                                <br /><h4>DEBTORS LISTING</h4> <br /><br /><br />
            ...
            foreach ($currencies as $record) {
                ...
                $entry += $this->custom->getCount('accounts_receivable', ['offset' => 'n', 'currency' => $record->currency]);
            }
            ...
            $file = 'debtor_listing_'.date('YmdHis').'.pdf';
            $style = $this->custom->populateMPDFStyle();
            $document = $style.$html;
    ...
    public function get_historical_listing($currency, $from_date, $to_date)
    {
        ...
        foreach ($customer_list as $customer_record) {
            // customer details
            $customer_data = $this->custom->getSingleRow('master_customer', ['code' => $customer_record->customer_code]);
            ...
            $table = 'accounts_receivable';
            $columns = '*, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount';
            $where = ['customer_code' => $customer_data->code, 'doc_date <' => $from_date];
            $group_by = 'REPLACE(doc_ref_no, "_sp_1", "")';
            $order_by = 'doc_date';
            $order_by_many = null;
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
            ...
            $table = 'accounts_receivable';
            $columns = '*, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount';
            $where = ['customer_code' => $customer_record->customer_code, 'doc_date >= ' => $from_date, 'doc_date <= ' => $to_date];
            $group_by = 'REPLACE(doc_ref_no, "_sp_1", "")';
            $order_by = null;
            $order_by_many = 'doc_date ASC, doc_ref_no ASC';
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
    ...
    public function print_historical_listing()
    {
        ...
        if ($post) {
            ...
            $html .= '<tr>
                                        ...
                                        <td style="border: none; text-align: right; vertical-align: bottom">
                                            <br /><h4>HISTORICAL DEBTOR LISTING</h4> <br /><br /><br />
                                        <strong>Date :</strong> '.date('d/m/Y').'</td>
                                    </tr>';
            $html .= '</table> <hr />';
            ...
            foreach ($currencies as $record) {
                ...
                // atleat one transaction should be not offsetted for this debtor, only then debtor details and entries will be displayed otherwise it no use to display the details
                $entry += $this->custom->getCount('accounts_receivable', ['currency' => $record->currency, 'doc_date >=' => $from_date, 'doc_date <=' => $to_date]);
            }
            ...
            $file = 'dbtr_hist_'.date('YmdHis').'.pdf';
            $style = $this->custom->populateMPDFStyle();
            $document = $style.$html;
    ...
    public function get_ageing($currency, $cutoff_date)
    {
        ...
        $html .= '<table class="table" style="width: 100%">';
        $html .= '<tr>
                    <th style="width: 250px">DEBTOR</th>
        ...
        foreach ($customer_list as $customer_record) {
            ...
            $table = 'accounts_receivable';
            $columns = 'doc_date, sign, f_amt';
            $where = ['offset' => 'n', 'customer_code' => $customer_record->customer_code, 'doc_date <= ' => $cutoff_date];
            $group_by = null;
            $order_by = null;
            $order_by_many = 'doc_date ASC, doc_ref_no ASC';
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by, $order_by_many);
            ...
            foreach ($stmt_list as $stmt_record) {
                ...
                if ($days >= 1 && $days <= 30) {
                    if ($stmt_record->sign == '+') {
                        $days_1_to_30_total += $stmt_record->f_amt;
                    } elseif ($stmt_record->sign == '-') {
                        $days_1_to_30_total -= $stmt_record->f_amt;
                    }
                } elseif ($days >= 31 && $days <= 60) {
                    if ($stmt_record->sign == '+') {
                        $days_31_to_60_total += $stmt_record->f_amt;
                    } elseif ($stmt_record->sign == '-') {
                        $days_31_to_60_total -= $stmt_record->f_amt;
                    }
                } elseif ($days >= 61 && $days <= 90) {
                    if ($stmt_record->sign == '+') {
                        $days_61_to_90_total += $stmt_record->f_amt;
                    } elseif ($stmt_record->sign == '-') {
                        $days_61_to_90_total -= $stmt_record->f_amt;
                    }
                } elseif ($days >= 91 && $days <= 120) {
                    if ($stmt_record->sign == '+') {
                        $days_91_to_120_total += $stmt_record->f_amt;
                    } elseif ($stmt_record->sign == '-') {
                        $days_91_to_120_total -= $stmt_record->f_amt;
                    }
                } elseif ($days > 120) {
                    if ($stmt_record->sign == '+') {
                        $days_120_plus_total += $stmt_record->f_amt;
                    } elseif ($stmt_record->sign == '-') {
                        $days_120_plus_total -= $stmt_record->f_amt;
                    }
                }
            ...
            $html .= '<tr>';
            $html .= '<td>'.$customer_data->name.' ('.$customer_record->customer_code.')</td>';
            ...
            $customer_total = $days_1_to_30_total + $days_31_to_60_total + $days_61_to_90_total + $days_91_to_120_total + $days_120_plus_total;
            $html .= '<td style="text-align: right; font-weight: bold;">';
            if ($customer_total >= 0) {
                $html .= number_format($customer_total, 2).' (DR)';
            } else {
                $html .= number_format(abs($customer_total), 2).' (CR)';
            }
            $html .= '</td>';
            $html .= '</tr>';
            ...
            $grand_customer_total += $customer_total;
        } // customer loops ends
        ...
        $html .= '<td style="text-align: right; font-weight: bold">';
        if ($grand_customer_total >= 0) {
            $html .= number_format($grand_customer_total, 2).' (DR)';
        } else {
            $html .= number_format(abs($grand_customer_total), 2).' (CR)';
        }
        $html .= '</td>';
    ...
    public function print_ageing()
    {
        ...
        if ($post) {
            ...
            $html .= '<tr>
                            ...
                            <td style="border: none; text-align: right; vertical-align: bottom;">
                                <br /><h4>DEBTORS AGEING</h4> <br /><br /><br />
                            <strong>Date :</strong> '.date('d/m/Y').'</td>
                        </tr>';
            $html .= '</table> <hr /> <br /><br />';
            ...
            $style = $this->custom->populateMPDFStyle();
            $file = 'dbtr_ageing_'.date('YmdHis').'.pdf';
            $document = $style.$html;
    ...
    public function batch_ob_edit($row_id = '')
    {
        ...
        if ($row_id != '') {
            $this->body_vars['customer_id'] = $row_id;
            $this->body_vars['customer_options'] = $this->custom->createDropdownSelect('master_customer', ['customer_id', 'name', 'code', 'currency_id'], 'Customer', ['( ', ') ', ''], ['active' => 1], ['customer_id' => $row_id]);

            $currency_id = $this->custom->getSingleValue('master_customer', 'currency_id', ['customer_id' => $row_id]);
            ...
            $this->body_vars['save_url'] = '/accounts_receivable/save_batch_ob';
            // $this->body_file = 'accounts_receivable/batch_ob_edit.php';
        }
    }
    ...
    public function manage_batch_ob($row_id = '') {
        ...
        if ($row_id != '') {
            ...
            $this->body_vars['customer_id'] = $row_id;
            $this->body_vars['customers'] = $this->custom->createDropdownSelect('master_customer', ['customer_id', 'name', 'code', 'currency_id'], 'Customer', ['( ', ') ', ''], ['active' => 1], ['customer_id' => $row_id]);

            $currency_id = $this->custom->getSingleValue('master_customer', 'currency_id', ['customer_id' => $row_id]);
            $currency = $this->custom->getMultiValues('ct_currency', 'code, rate', ['currency_id' => $currency_id]);
            $this->body_vars['customer_currency'] = $currency->code;
            ...
            $this->body_vars['customer_currency'] = '';
    ...
    // data patch opening balance transactions in accounts_receivable.TBL
    public function data_patch()
    {
        ...
        if ($post) {
            ...
            $this->body_vars['customer_code'] = $post['customer'];
            $this->body_vars['customers'] = $this->custom->createDropdownSelect('master_customer', ['code', 'name', 'code', 'currency_id'], 'Customer', ['( ', ') ', ''], ['active' => 1], ['code' => $post['customer']]);
            ...
            $currency_id = $this->custom->getSingleValue('master_customer', 'currency_id', ['code' => $post['customer']]);
            $currency = $this->custom->getMultiValues('ct_currency', 'code, rate', ['currency_id' => $currency_id]);
            $this->body_vars['customer_currency'] = $currency->code;
            $this->body_vars['currency_rate'] = $currency->rate;
            ...
            $this->body_vars['save_url'] = '/accounts_receivable/save_patched_data';
        }
    }

    // datapatch Opening Blanace Tranasctions in AR.TBL
    public function save_patched_data()
    {
        ...
        if ($post) {
            ...
            for ($i = 0; $i <= $total_items - 1; ++$i) {
                ...
                $ob_data['f_amt'] = $post['foreign_amount'][$i];
                $ob_data['total_amt'] = $post['local_amount'][$i];
                $ob_data['sign'] = $post['sign'][$i];
                $ob_data['remarks'] = $post['remarks'][$i];

                $ar_id = $post['entry_id'][$i];
                $updated[] = $this->custom->updateRow('accounts_receivable', $ob_data, ['customer_code' => $customer_code, 'ar_id' => $ar_id]);
            }
    ...
    public function save_batch_ob()
    {
        ...
        if ($post) {
            ...
            $total_items = count($post['entry_id']);
            for ($i = 0; $i <= $total_items - 1; ++$i) {
                $batch_data['customer_id'] = $customer_id;
                ...
                $ar_ob_id = $post['entry_id'][$i];
                $updated[] = $this->custom->updateRow('ar_open', $batch_data, ['ar_ob_id' => $ar_ob_id]);
            }
        ...
        redirect('accounts_receivable/batch_ob_listing');
    }

    public function delete_all_ar_ob()
    {
        $this->db->select('ar_ob_id');
        $this->db->from('ar_open');
        $this->db->where(['status' => 'C']);
        $query = $this->db->get();
        $delete_ob_id = $query->result();

        foreach ($delete_ob_id as $key => $value) {
            $deleted[] = $this->custom->deleteRow('ar_open', ['ar_ob_id' => $value->ar_ob_id]);
        }
        ...
        redirect('accounts_receivable/opening_balance_list/confirmed/delete');
    }
    ...
    public function current_statement()
    {
        is_logged_in('admin');
        has_permission();
        $this->body_vars['customer_options'] = $this->custom->createDropdownSelect('master_customer', ['customer_id', 'name', 'code', 'currency_id'], '', ['( ', ') ', ' '], ['active' => 1]);
        $this->body_file = 'accounts_receivable/debtor_current_statement.php';
    }
    ...
    public function current_listing()
    {
        ...
        $this->body_file = 'accounts_receivable/debtor_current_listing.php';
    }
    ...
    public function offset()
    {
        ...
        $status = $this->custom->updateRow('accounts_receivable', ['offset' => 'o'], ['doc_date <=' => $date_to_offset, 'settled' => 'y']);
    ...
    public function print_batch_ob()
    {
        ...
        if ($_GET['rowID'] !== null) {
            $customer_id = $_GET['rowID'];
            $where = ['customer_id' => $customer_id, 'status' => $ob_bf_type];
        }
        $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by);
        // print_r($this->db->last_query());
        $list = $query->result();
        foreach ($list as $row) {
            $customer_data = $this->custom->getMultiValues('master_customer', 'name, code', ['customer_id' => $row->customer_id]);
            ...
            $html .= $customer_data->name.' ('.$customer_data->code.')';
            $html .= '<tr>';
            $html .= '<td colspan="6" style="border: none; font-weight: bold">'.$customer_data->name.' ('.$customer_data->code.')</td>';
            $html .= '</tr>';

            $html .= '<tr>
                <th>Date</th>
                <th>Reference</th>
                <th style="width: 150px">FAMT $</th>
                <th style="width: 150px">SGD $</th>
                <th>Sign</th>
            ...
            foreach ($record_list as $record) {
                ...
                $html .= '<td>'.number_format($record->foreign_amount, 2).'</td>';
                $html .= '<td>'.number_format($record->local_amount, 2).'</td>';
        ...
        $file = 'ar_ob_list_'.date('YmdHis').'.pdf';
        $this->custom->printMPDF($file, $html);
    }

    public function print_ar_ob()
    {
        $html = '';
        $html .= $this->custom->populateMPDFStyle();
        $html .= '<div style="width: 100%; margin: auto; text-align: center;"><h3>OPENING BALANCE</h3></div>';
        ...
        $i = 0;
        $table = 'accounts_receivable';
        $columns = 'customer_code, currency';
        $group_by = 'customer_code';
        $order_by = null;
        $order_by_many = 'customer_code ASC, doc_date ASC, doc_ref_no ASC';
        $where = ['tran_type' => 'OPBAL'];
        if ($_GET['rowID'] !== null) {
            $customer_id = $_GET['rowID'];
            $customer_code = $this->custom->getSingleValue('master_customer', 'code', ['customer_id' => $customer_id]);
            $where = ['customer_code' => $customer_code, 'tran_type' => 'OPBAL'];
        }
        $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by);
        // print_r($this->db->last_query());
        $list = $query->result();
        foreach ($list as $row) {
            $customer_data = $this->custom->getMultiValues('master_customer', 'name, code', ['code' => $row->customer_code]);
            ...
            $html .= '<tr>';
            $html .= '<td colspan="6">'.$customer_data->name.' ('.$customer_data->code.') | Currency : '.$row->currency.'</td>';
            $html .= '</tr>';
            ...
            $order_by = null;
            $order_by_many = 'doc_date ASC, doc_ref_no ASC';
            $where = ['customer_code' => $row->customer_code, 'tran_type' => 'OPBAL'];
            $query = $this->ar_model->get_tbl_data($table, $columns, $where, $group_by, $order_by);
            $record_list = $query->result();
            foreach ($record_list as $record) {
                $html .= '<tr>';
                ...
                $html .= '<td>'.number_format($record->f_amt, 2).'</td>';
    ...
    public function print_audit()
    {
        ...
        if ($post) {
            ...
            $html .= '</td>';
            $html .= '<td style="border: none; text-align: right; padding-right: 0;"><h3>AR AUDIT LISTING</h3></td>';
            $html .= '</tr>';
            ...
            $html .= '<tr>
                        ...
                        <th>Currency</th>
                        <th>Sign</th>
                        <th style="text-align: right; width: 140px;">Foreign Amount</th>
                        <th style="text-align: right; width: 140px;">'.$this->custom->getDefaultCurrency().' Amount</th>
                    </tr>';
            $i = 0;
            $bf_total_per_subledger = 0;
            $sql = 'SELECT *, REPLACE(doc_ref_no, "_sp_1", "") AS original_doc_ref, sum(total_amt) AS total_local_amount, sum(f_amt) AS total_foreign_amount FROM accounts_receivable WHERE tran_type = "'.$transaction_type.'" GROUP BY REPLACE(doc_ref_no, "_sp_1", "") ORDER BY doc_date '.$transaction_order.', doc_ref_no ASC';
            $query = $this->db->query($sql);
            $ar_data = $query->result();
            foreach ($ar_data as $key => $value) {
                $customer_name = $this->custom->getSingleValue('master_customer', 'name', ['code' => $value->customer_code]);
                ...
                $html .= '<td>('.$value->customer_code.') '.$customer_name.'</td>';
                $html .= '<td style="text-align: center">'.$value->currency.'</td>';                
                $html .= '<td style="text-align: center">'.$value->sign.'</td>';
            ...
            if ($i == 0) {
                ...
                $html .= '<tr><td colspan="7" style="border: none" height="20"></td></tr>';
                ...
                $sql_bf_gl = 'SELECT sign, total_amount FROM gl WHERE accn = "CA001" AND tran_type = "'.$transaction_type.'" ';
                ...
                foreach ($data_gl as $key => $value) {
                    if ($value->sign == '+') {
                        $bf_total_per_gl += $value->total_amount;
                    } else {
                        $bf_total_per_gl -= $value->total_amount;
                    }
                }
            ...
            $file = 'ar_audit_'.date('YmdHis').'.pdf';
            $this->custom->printMPDF($file, $document);
    ...
    public function df_backup($tables = [])
    {
        ...
        $file_name = 'ar_'.date('j-F-Y_H-i-s').'.sql';

        $CI->load->dbutil();
        $prefs = [
            'tables' => ['ar_open', 'accounts_receivable'],
            'format' => 'sql',           // sql, txt
    ...
    public function df_restore($action = 'form')
    {
        ...
        if ($data['status']) {
            ...
            set_flash_message('message', 'success', 'Accounts Receivable datafile restored');
        } else {
            set_flash_message('message', 'warning', $data['error']);
        }
        redirect('accounts_receivable/', 'refresh');        
    }
    ...
    public function df_zap()
    {
        is_logged_in('admin');
        has_permission();
        $this->body_file = 'accounts_receivable/blank.php';
        zapAR();
        redirect('accounts_receivable/', 'refresh');
    }
}
