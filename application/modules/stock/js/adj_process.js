$(function() { // document starts
   ...
   $(document).on("change", "#ref_no", function(e) {
      ...
      if(ref_no !== "") {
         ...
         // Double Check Reference in Stock and OB batch Tables
         $.post('/stock/ajax/double_adj', {
            ref_no: $("#ref_no").val()
         }, function(ref) {
            if (ref > 0) {
               double_ref = 1;
   ...
   $(document).on('click', '.dt_edit', function () {
      ...
      $('#adj_id').val($('#adj_id_'+row_number).val());
      ...
      $.confirm({
         ...
         buttons: {
            yes: {
               ...
               action: function() {
                  $.post('/stock/ajax/delete_adj_entry', {
                     adj_id: $('#adj_id_'+row_number).val()
                  }, function (status) {
...
function clear_inputs() {
   ...
   $('#uom').val('');
   $('#quantity').val('');
   $('#sign').select2("destroy").val('').select2();
   $('#remarks').val('');
}
...
function save_entry() {
   ...
   $.post('/stock/ajax/save_adj_entry', {
      adj_id: adj_id,
      doc_date: doc_date,
      ref_no: ref_no,
...
function manage_entry() {
   ...
   $row.find('input.adj_id').val($('#adj_id').val());
   ...
   $row.find('input.sign').val($('#sign').val());
