   ...
   <div class="container-fluid">
      ...
      <div class="row">
         <div class="col-lg-12">
            ...
            <input type="hidden" id="supplier_currency" value="<?php echo $supplier_currency; ?>" />
            ...
            <form autocomplete="off" id="form_" method="post" action="<?php echo $save_url; ?>">
               <div class="card card-default">
                  <div class="card-header">
                     <h5>OB Data Patch</h5>
                     <a href="/accounts_payable/" class="btn btn-outline-secondary btn-sm float-right">Back</a>
                  </div>
                  <div class="card-body">
                     <div class="row">
                        <div class="col-md-4">
                           <label class="control-label">To : </label><br />
                           <select name="supplier" id="supplier" class="form-control" style="pointer-events: none; background: #f5f5f5" onclick="return false;" onkeydown="return false;">
                              <?php echo $suppliers; ?>
                           </select>
                        </div>
                     </div>
                           ...
                           <table id="tbl_items" class="table table-custom" style="min-width: 1400px; width: 100%;">
                              <thead>
                                 <tr>
                                    ...
                                    <th class="w-200">Amount <span class="f_curr" style="display: <?php echo $supplier_currency == 'SGD' ? 'none' : 'inline-block'; ?>">(<?php echo $supplier_currency; ?>)</span> $</th>
                                    <th class="w-200 dv_local" style="display: <?php echo $supplier_currency == 'SGD' ? 'none' : 'table-cell'; ?>">Amount <strong>(SGD)</strong> $</th>
                                    <th>Remarks</th>
                                    <th></th>
                                 </tr>
                              </thead>
                              ...
                              $this->db->where(['supplier_code' => $supplier_code, 'tran_type' => 'OPBAL']);
                              if($ref_no != '') {
                                 $this->db->where(['supplier_code' => $supplier_code, 'doc_ref_no' => $ref_no, 'tran_type' => 'OPBAL']);
                              }
                                 ...
                                 <tr id="<?php echo $i; ?>">
                                    ...
                                    <td>
                                       <input 
                                          ...
                                          value="<?php echo $value->fa_amt; ?>"
                                          class="form-control foreign_amount" readonly />
                                    </td>
                                    
                                    <td class="dv_local" style="display: <?php echo $supplier_currency == 'SGD' ? 'none' : 'table-cell'; ?>">
                                       <input 
                                          type="number" 
               ...
               ?>
                  ...
                  <div class="card-footer">
                     <a href="/accounts_payable/" class="btn btn-info btn-sm">Cancel</a>                  
...
$(function() { // document starts
   ...
   var supplier_code = $("#supplier").val();
   ...
   $(document).on("change", "#ref_no", function() {
      ...
      if(current_ref !== "" && current_ref !== original_ref) {
         ...
         $.post('/accounts_payable/ajax/double_ref', {
      ...
      $.confirm({
         ...
         buttons: {
            yes: {
               ...
               action: function() {
                  $.post('/accounts_payable/ajax/delete_ap_ob_entry', {
                     supplier_code: supplier_code,
                     ap_id: $('#entry_id_'+row_number).val()
                  }, function (status) {
