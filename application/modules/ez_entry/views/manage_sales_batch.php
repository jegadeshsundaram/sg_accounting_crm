            ...
            <form autocomplete="off" id="frm_" method="post">
               ...
               <div class="card card-default">
                  <div class="card-header">
                     <h5 style="float: left; margin-top: 8px;">Credit Sales</h5>
                     <a href="/ez_entry/batch_sales" class="btn btn-outline-dark btn-sm float-right">
                  ...
                  <div class="card-body">
                     ...
                     <div class="row">
                        <div class="col-md-6">
                           <div class="row form-group">
                              <label class="control-label col-md-4">Customer<span class="cl-red">*</span></label>
                              <div class="col-md-8">
                                 <select id="customer" name="customer" class="form-control customer">
                                    <?php echo $customers; ?>
                                 </select>
                              </div>
                           </div>
                           ...
                           <table id="tbl_items" class="table table-custom" style="min-width: 1350px; width: 100%; display: <?php echo $page == 'edit' ? '' : 'none'; ?>">
                              ...
                              <?php
                                 if ($page == 'edit') {
                                    ...
                                    $query = $this->db->get();
                                    $sl_entries = $query->result();
                                    foreach ($sl_entries as $value) { ?>
                                 <tr id="<?php echo $i; ?>">
                                    <td class="w-140">
                                       <input 
                                          type="hidden" 
                                          id="entry_id_<?php echo $i; ?>" name="entry_id[]" 
                                          value="<?php echo $value->sb_id; ?>"
                                          class="entry_id" />
                                    ...
                                    <td class="w-250">
                                       <input 
                                          type="hidden" 
                                          id="sales_accn_<?php echo $i; ?>" name="sales_accn[]" 
                                          value="<?php echo $value->sales_accn; ?>"
                                          class="sales_accn" />

                                       <?php $accn_desc = $this->custom->getSingleValue('chart_of_account', 'description', ['accn' => $value->sales_accn]); ?>
                                       <input 
                                          type="text" 
                                          id="sales_accn_desc_<?php echo $i; ?>"  
                                          value="<?php echo $value->sales_accn .' : '. $accn_desc; ?>"
                                          class="form-control-dsply sales_accn_desc" />
                                    </td>
   ...
   <tbody>
      <tr id="0">
         ...
         <td class="w-250">
            <input 
               type="hidden" 
               id="sales_accn_0" name="sales_accn[]" 
               class="sales_accn" />
            
            <input 
               type="text" 
               id="sales_accn_desc_0"  
               class="form-control-dsply sales_accn_desc" />
         </td>
            ...
            <div class="card-body">
               ...
               <div class="row mt-10">
                  <div class="col-md-12 col-12">
                     <label class="control-label">Sales Account<span class="cl-red">*</span></label>
                     <select id="sales_accn" name="sales_accn" class="form-control sales_accn">
                        <?php echo $sales_accns; ?>
                     </select>
                  </div>
               </div>
            </div>
...
<script>
   ...
   $(function() {
      ...
      $(document).on("change", "#ref_no", function() {
         ...
         $.post('/ez_entry/ajax/double_sales', {
            ref_no: ref_no
         }, function(ref) {
            if (ref > 0) {
               duplicate_ref = true;
      ...
      $(document).on('click', '.dt_edit', function () {
         ...
         $('#sales_accn').select2("destroy");
         $('#sales_accn').val($('#sales_accn_'+row_number).val());
         $('#sales_accn').select2();
         ...
         $.confirm({
            ...
            buttons: {
               yes: {
                  ...
                  action: function() {
                     $.post('/ez_entry/ajax/delete_sales_entry', {
      ...
      $("#btn_save_to_batch").on('click', function (e) {
         if($('#tbl_items > tbody > tr').length > 0 && isFormValid()) {
            ...
            toastr.success("Invoice saved to batch!");
            window.location.href = '/ez_entry/batch_sales';
         }
      });
   ...
   function isModalValid() {
      ...
      if($('#foreign_amount').val() == "") {
         ...
         valid = false;
      } else if($('#sales_accn').val() == "") {
         $("#sales_accn").select2('open');
         valid = false;
      }
   ...
   function save(row_number) {
      ...
      var sales_accn = $("#sales_accn"+row_number).val();
      ...
      $.post('/ez_entry/ajax/save_sales', {
         entry_id: entry_id,
         doc_date: doc_date,
         ref_no: ref_no,
         customer: customer,
         sales_accn: sales_accn,
   ...
   function manage_entry() {
      ...
      $row.find('input.sales_accn').val($('#sales_accn').val());
      $row.find('input.sales_accn_desc').val($("#sales_accn>option:selected").text());
