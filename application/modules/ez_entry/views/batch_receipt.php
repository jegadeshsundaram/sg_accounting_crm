      ...
      <div class="modal-content">
         <div class="card card-default" style="margin-bottom: 0rem;">
            <div class="card-body">
                ...
                <div class="row form-group">
                    <div class="col-12">
                        <label for="customer" class="control-label">Customer<span class="cl-red">*</span></label>
...
<script type="text/javascript">
   ...
   $(function() {
      table = $('#dt_batch').DataTable({
         'processing': true,
         'ajax': {
            'url': '/ez_entry/datatable/ajax_list/batch_receipt',
            'type': 'post'
         },
         'columnDefs': [
      ...
      $("#btn_delete_all").on('click',function() {
         if(table.rows().eq(0).length > 0) {
            ...
            url = '/ez_entry/ajax/delete_receipt';
      ...
      $("#btn_post_all").on('click', function() {
         if(table.rows().eq(0).length > 0) {
            ...
            url = '/ez_entry/ajax/post_receipt';
      ...
      $("#btn_print_audit").on('click', function() {
         if(table.rows().eq(0).length > 0) {
            url = '/ez_entry/print_receipt_audit_trail';
            print_all(url);
         }
      });
      ...
      $('#dt_batch').on('click', 'tbody .dt_view', function () {
         ...
         $.post('/ez_entry/ajax/get_receipt_view', {
            entry_id: rowID
         }, function (data) {
      ...
      $('#dt_batch').on('click', 'tbody .dt_edit', function () {
         ...
         $('#entry_id').val(rowID);
         $.post('/ez_entry/ajax/get_receipt', {
            rowID: rowID
         }, function (receipt) {
            var obj = $.parseJSON(receipt);

            $('#doc_date').val(obj.receipt['doc_date']);
            $('#ref_no').val(obj.receipt['ref_no']);
            $('#original_ref_no').val(obj.receipt['ref_no']);
      
            $('#customer').select2("destroy");
            $('#customer').val(obj.receipt['customer_id']);
            $('#customer').select2();

            $('#currency').val(obj.currency);
            $('#exchange_rate').val(obj.receipt['exchange_rate']);
            if(obj.currency == "SGD") {
               $('.dv_currency').hide();
               $('.dv_local_amount').hide();
            } else {
               $('.f_curr').text(obj.receipt['currency']);
               $('.dv_currency').show();
            ...
            $('#foreign_amount').val(obj.receipt['foreign_amount']);
            $('#local_amount').val(obj.receipt['local_amount']);
            $('#remarks').val(obj.receipt['remarks']);

            $('#bank').select2("destroy");
            $('#bank').val(obj.receipt['bank_accn']);
            $('#bank').select2();

            $('#foreign_bank').select2("destroy");
            $('#foreign_bank').val(obj.receipt['fb_accn']);
            $('#foreign_bank').select2();

            if(obj.receipt['bank_accn'] == "CA110") {
               $('.dv_foreign_bank').show();
            } else {
      ...
      $('#dt_batch').on('click', 'tbody .dt_delete', function () {
         ...
         $.confirm({
            title: '<i class="fa fa-info"></i> Delete Receipt',
            content: 'Are you sure to delete?',
            buttons: {
               yes: {
                  btnClass: 'btn-warning',
                  action: function() {
                     $.post('/ez_entry/ajax/delete_receipt', {
                        rowID: rowID
                     }, function (res) {
                        if(res == "error") {
                           toastr.error("ERROR");
      ...
      $(document).on("change", "#ref_no", function() {
         ...
         $(".error-ref").hide();
         $.post('/ez_entry/ajax/double_receipt', {
            ref_no: ref_no
         }, function(ref) {
            if (ref > 0) {
               duplicate_ref = true;
   ...
   function save() {
      $.post('/ez_entry/ajax/save_receipt', {
