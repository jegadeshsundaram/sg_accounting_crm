...
<div class="content">
   <div class="container-fluid">
      ...
      <div class="row">
         <div class="col-lg-12">
            <input type="hidden" id="recon_bank" value="<?php echo $recon_bank; ?>" />
            <form autocomplete="off" id="form_" method="post">
               <input type="hidden" name="tran_type" value="<?php echo strtoupper($transaction_type); ?>" />
               <div class="card card-default">
                  <div class="card-header">
                     <h5>
                        <?php if ($transaction_type == 'eo') { ?>
                           Input Errors & Omission 
                        <?php } elseif ($transaction_type == 'td') { ?>
                           Input Timing Differences
                        <?php } ?>
                     </h5>
                     <a href="/bank_reconciliation/input_options" class="btn btn-outline-dark btn-sm float-right">
                  ...
                  <div class="card-body">
                     ...
                     <?php if ($transaction_type == 'td') { ?>
                        <div class="row form-group">
                           <div class="col-md-8">
                              <label class="control-label" for="ref">Transaction : </label><br />
                     ...
                     <div class="row table-responsive">
                        <div class="col-md-12">
                           <table id="tbl_items" class="table" style="min-width: 1400px; width: 100%;">
                              <thead>
                                 <?php
                                    $entries = $this->custom->getCount('bank_recon_current', ['bank_accn' => $bank, 'tran_type' => $transaction_type, 'accounted' => 'n']);
               ?>
               ...
               $this->db->where(['bank_accn' => $bank, 'tran_type' => $transaction_type, 'accounted' => 'n']);
               $query = $this->db->get();
               ...
               ?>
                  ...
                  <div class="card-footer">
                     <a href="/bank_reconciliation/input_options" class="btn btn-info btn-sm">Cancel</a>
...
<script>
   ...
   $(function() {
      ...
      $("#ref").change(function(event) {
         entry_id = $("#ref option:selected").val();
         ref_text = $("#ref option:selected").text();
         var ref_data = ref_text.split('|');
         if(entry_id !== "") {

            var entry_exists = false;
            $("#tbl_items").find('.doc_ref').each(function() {
               console.log("ref_data[1] >>> "+ref_data[1]);
               console.log("this.val() >>> "+$(this).val());
               if($.trim(ref_data[1]) == $.trim($(this).val())) {
                  entry_exists = true;
                  $('#errorAlertModal .modal-title').html("Already Selected!");
                  $('#errorAlertModal .modal-body').html("The selected transaction with reference " + ref_data[1] + " is already used here. <br /><br />Please select other transaction");
            ...
            var text = "<span style='color: brown; width: 80px; display: inline-block;'>Date :</span> " + ref_data[0] + "<br /><span style='color: brown; width: 80px; display: inline-block;'>Reference :</span> " + ref_data[1] + "<br /><span style='color: brown; width: 80px; display: inline-block;'>Remarks: </span>" + ref_data[2] + "<br /><span style='color: brown; width: 80px; display: inline-block;'>Amount : </span>" + ref_data[3];
            $.confirm({
               title: '<i class="fa fa-info"></i> Confirm Transaction',
               content: text,
               buttons: {
                  yes: {
                     btnClass: 'btn-warning',
                     action: function() {
                        populate_data_by_reference = true;
                        add_entry();
                     }
                  },
                  no: {
