...
$(function() {
   ...
   if(page == "new") {
      ...
      var ref_no = $('#invoice_ref_no').val();
      var ref_parts = ref_no.split('.');
      $.post('/invoice/ajax/invoice_new_reference', {
   ...
   $("#customer_id").change(function() {
      ...
      if (customer_id !== "") {
         $.post('/invoice/ajax/get_customer_details', {
            ...
            if(obj.credit_limit > 0) {
               ...
               $('.credit_terms').html(obj.credit_terms + " days");
               $('.credit_limit').html(obj.customer_currency +" "+ obj.credit_limit.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
               $('.amount_utilized').html(obj.customer_currency +" "+ obj.amount_utilized.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));

               var cl =  Number(obj.credit_limit);
               var au = Number(obj.amount_utilized);

               var avail = -cl + au;
               avail = avail * -1;
               $('.amount_available').html(obj.customer_currency +" "+ avail.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
               if(cl < au) {
                  $('.amount_available').addClass('blinking');
               } else {
                  $('.amount_available').removeClass('blinking');
               }
               $('#creditModal').modal();
            } else {
   ...
   $('#btn-credit-proceed').click(function(event) {
      $('#creditModal').modal('hide');
      $("#employee_id").select2("open");
   });

   $('#btn-credit-abort').click(function(event) {
   ...
   $('#btn-confirm-yes').click(function() {
      $.post('/invoice/ajax/set_unit_price', {
         tbl: set_price_tbl,
         customer_id: customer_id,
   ...
   $(document).on('change', '.unit_price', function() {
      ...
      $('#unitPriceConfirmModal').modal();
   });

   $("#btn-unitprice-confirm-no").click(function() {
      process_item_amount(processing_row_number);
      $("#unitPriceConfirmModal").modal('hide');
   });

   $("#btn-unitprice-confirm-yes").click(function() {
      $.post('/invoice/ajax/set_unit_price', {
         tbl: "customer_price",
         customer_id: customer_id,
         billing_id: $('#billing').val(),
         billing_price_per_uom: $('#unit_price').val()
      }, function (data) {
         if(data == "error") {
         } else {
            process_item_amount();
   ...
   $(document).on('change', '#gst_category', function() {
      ...
      if(gst_code !== "") {
         ...
         if(gst_code == "SRCA-S") {
            $.post('/invoice/ajax/get_customer_gst_number', {
               customer_id: customer_id
            }, function (data) {
         ...
         $.post('/invoice/ajax/get_gst_details', {
            gst_code: gst_code
         }, function (data) {
            if(data !== "") {
               var obj = $.parseJSON(data);
   ...
   $("#btn-special-gst-no").click(function() {
      ...
      $.confirm({
         title: '<i class="fa fa-info"></i> Abort Invoice',
   ...
   $(document).on('click', '.btn_add_details', function() {
      ...
      var item_details = "";
      if(billing_id !== "") {
         $.post('/invoice/ajax/get_billing_details', {
            billing_id: billing_id
         }, function(data) {
            var obj = $.parseJSON(data);
   ...
   $("#btn_submit").on('click', function (e) {
      ...
      $("#form_").attr("action", '/invoice/save'+action);
