...
class Ion_auth_model extends CI_Model
{
    ...
    public function get_gst_merchant()
    {
        $query = $this->db->select('gst_reg_no')
                          ->where('code', 'CP')
                          ->get($this->tables['company_profile']);

        $gst_merchant_data = $query->row();

        if ($query->num_rows() !== 1) {
            return false;
        }

        if ($gst_merchant_data->gst_reg_no !== null && $gst_merchant_data->gst_reg_no !== '') {
            return true;
        } else {
            return false;
        }
     ...
     * @author Mathew
     **/
    public function update_admin_credentials($identity, $username, $new_password)
    {
        $this->trigger_events('pre_change_password');
        $this->trigger_events('extra_where');

        $query = $this->db->select('id, username, password, salt')
            ->where('id', $identity) // $identity is id from users.tbl updated by Jega on FEB 16, 2023
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
        ...
        $successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data, ['id' => $identity]);
